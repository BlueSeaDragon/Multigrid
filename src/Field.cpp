...
void BosonField::init_uniform(double const A) {
    ...
    A_ = Eigen::VectorXd::Constant(dim * dim * 2, A);
}

void BosonField::init_staggered(double diff) {
    int dim = _mesh.get_dim();
    A_ = Eigen::VectorXd::Constant(dim * dim * 2, 0.);
...
double BosonField::Q() {
    ...
    for (int pos=0; pos<_mesh.get_size(); pos++) {
        ...
        double const A_sum = A_(2*pos) + A_(2*pos_up+1) - A_(2*pos_right) - A_(2*pos+1);
        q_sum += A_LAT*A_sum;
    }
...
void BosonField::printA() {
    ...
    for (int i=0; i<dim; i++) {
        std::cout<<" ";
        for (int j=0; j<dim; j++) {
            std::cout<<A_(2*(i*dim+j)+1)<<"  ";
        }
        std::cout<<std::endl;

        for (int j=0; j<dim; j++) {
            std::cout<<A_(2*(i*dim+j)) <<"  ";
        }
        std::cout<<std::endl;
    }
}
